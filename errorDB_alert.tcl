...
puts "Lindex 3: [lindex [split [lindex $errorDbDump $lastIndex]] 3]"
if { [lindex [split [lindex $errorDbDump $lastIndex]] 2] == "" } {
	set mid [string map {\[ "" \] ""} [lindex [split [lindex $errorDbDump $lastIndex]] 3]]
} else {
...
smtp::sendmessage $token -header [list Subject "***TEST*** - A message has entered the error database for $site"] \

...
set columns "\[\{\"name\":\"thread\",\"title\":\"Thread\",\"breakpoints\":\"sm\"\},"
lappend columns "\{\"name\":\"source\",\"title\":\"Source\"\},"
lappend columns "\{\"name\":\"destination\",\"title\":\"Destination\"\},"
lappend columns "\{\"name\":\"type\",\"title\":\"Type\"\},"
lappend columns "\{\"name\":\"environment\",\"title\":\"Environment\"\},"
lappend columns "\{\"name\":\"status\",\"title\":\"Status\"\}\n\]"
    ...
    foreach conn $connList {
        ...
        if { [string match "EMR_O*" $conn] || [string match "EMR_I*" $conn] || [lsearch -exact $exceptions $conn] > -1 } {
            ...
            if {[lindex $serverIp 1] == "ESTABLISHED" } {
                ...
                lappend rows "\{\"thread\":\"$conn\",\"source\":\"[string trim $nSource]\",\"destination\":\"[string trim $nDest]\",\"type\":\"[string trim $nMsgType]\",\"environment\":\"[lindex [split [lindex $serverName 0] {.}] 0]\",\"status\":\"[lindex $serverIp 1]\"\},"
            } else {
                ...
                lappend rows "\{\"thread\":\"$conn\",\"source\":\"[string trim $nSource]\",\"destination\":\"[string trim $nDest]\",\"type\":\"[string trim $nMsgType]\",\"environment\":\"[lindex [split [lindex $serverName 0] {.}] 0]\",\"status\":\"DISCONNECTED\"\},"
            }
...
set jsonColumnsFile "/hci/cis6.2/integrator/contrib/html/milliColumns.json"
set jsonRowsFile "/hci/cis6.2/integrator/contrib/html/milliRows.json"
